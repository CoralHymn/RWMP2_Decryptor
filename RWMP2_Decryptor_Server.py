...
def web_main():
    ...
    try:
        # 第一步：执行逆向替换预处理
        processed_data = reverse_replace_in_zip(str(zip_path))

        # 第二步：使用处理后的数据进行修复
        repair_tool = AdvancedZipRepair(str(zip_path), processed_data)
        ...
        final_zip_filename = zip_path.stem + "_最终修复.zip"  # 这是用户下载时看到的文件名
        ...
        def cleanup_all_temp_files():
            ...
            try:
                ...
                if zip_path.exists():
                    os.remove(zip_path)
                    print(f"✅ 已清理临时上传文件: {zip_path}")
                else:
                    print(f"⚠️ 临时上传文件已不存在: {zip_path}")
